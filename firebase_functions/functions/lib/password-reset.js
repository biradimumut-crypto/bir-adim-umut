 ...
 */
exports.sendPasswordResetCode = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    var _a;
    const email = (_a = request.data) === null || _a === void 0 ? void 0 : _a.email;
    if (!email || typeof email !== "string") {
        throw new https_1.HttpsError("invalid-argument", "Email adresi gerekli");
    }
    ...
    try {
        ...
        try {
            ...
            throw new https_1.HttpsError("not-found", "Bu email ile kayıtlı kullanıcı bulunamadı");
        }
        // Son 1 dakika içinde kod gönderilmiş mi kontrol et
        const recentCodes = await db.collection("password_reset_codes")
            .where("email", "==", normalizedEmail)
            .where("created_at", ">", admin.firestore.Timestamp.fromDate(new Date(Date.now() - 60 * 1000)))
            .get();
 ...
 * 🚨 P1-2: App Check enforcement aktif
 */
exports.resetPasswordWithCode = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    ...
    if (!newPassword || typeof newPassword !== "string" || newPassword.length < 6) {
        throw new https_1.HttpsError("invalid-argument", "Şifre en az 6 karakter olmalı");
    }
    const normalizedEmail = email.toLowerCase().trim();
    try {
        // En son kodu bul
        ...
        if (codesSnapshot.empty) {
            throw new https_1.HttpsError("not-found", "Geçerli sıfırlama kodu bulunamadı. Yeni kod isteyin.");
        }
        ...
        throw new https_1.HttpsError("internal", "Şifre sıfırlama başarısız");
    }
});
