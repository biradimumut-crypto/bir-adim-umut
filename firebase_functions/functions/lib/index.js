...
exports.createTeam = functions.https.onCall(async (data, context) => {
    ...
    if (!teamName || teamName.trim().length < 3) {
        throw new functions.https.HttpsError("invalid-argument", "Takım adı en az 3 karakter olmalıdır.");
    }
    try {
...
exports.joinTeamByReferral = functions.https.onCall(async (data, context) => {
    ...
    if (!referralCode || referralCode.trim().length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "Referral kod gereklidir.");
    }
    try {
        ...
        const teamQuery = await db
            ...
            .get();
        if (teamQuery.empty) {
            throw new functions.https.HttpsError("not-found", "Bu referral kod ile bir takım bulunamadı.");
        }
        ...
        if (existingMember.exists) {
            throw new functions.https.HttpsError("already-exists", "Siz zaten bu takımın üyesisiniz.");
        }
        // Kullanıcı başka bir takımda mı kontrol et
        const userDoc = await db.collection("users").doc(userId).get();
        if ((_b = userDoc.data()) === null || _b === void 0 ? void 0 : _b.current_team_id) {
            throw new functions.https.HttpsError("invalid-argument", "Siz zaten başka bir takımın üyesisiniz. Lütfen önce o takımdan ayrılın.");
        }
...
exports.inviteUserToTeam = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (teamData.leader_uid !== leaderUid) {
            throw new functions.https.HttpsError("permission-denied", "Sadece takım lideri üye davet edebilir.");
        }
        ...
        const existingMember = await teamDoc.ref
            ...
            .get();
        if (existingMember.exists) {
            throw new functions.https.HttpsError("already-exists", "Bu kullanıcı zaten takımda.");
        }
        // Lider bilgisini al
        ...
        await db
            ...
            created_at: admin.firestore.FieldValue.serverTimestamp(),
            responded_at: null,
            sender_name: (leaderData === null || leaderData === void 0 ? void 0 : leaderData.full_name) || "Bilinmiyor",
        ...
        if (targetUserTokens.length > 0) {
            await admin.messaging().sendMulticast({
                ...
                notification: {
                    ...
                    body: `${leaderData === null || leaderData === void 0 ? void 0 : leaderData.full_name} sizi takıma davet etti.`,
                },
                data: {
                    teamId,
                    notificationId,
...
exports.acceptTeamInvite = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (notificationData.notification_status !== "pending") {
            throw new functions.https.HttpsError("invalid-argument", "Bu bildirim zaten cevap alınmış.");
        }
        // Takımı al
        const teamDoc = await db.collection("teams").doc(teamId).get();
        if (!teamDoc.exists) {
...
async function getDeviceTokens(userId) {
    var _a;
    const userDoc = await db.collection("users").doc(userId).get();
    const tokens = ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.device_tokens) || [];
 ...
 */
exports.carryOverDailySteps = functions.pubsub
        ...
        for (const userDoc of usersSnapshot.docs) {
            try {
                ...
                if (yesterdayStepsDoc.exists) {
                    ...
                    const dailySteps = (stepData === null || stepData === void 0 ? void 0 : stepData.daily_steps) || 0;
                    const convertedSteps = (stepData === null || stepData === void 0 ? void 0 : stepData.converted_steps) || 0;
 ...
 */
exports.resetDailyTeamSteps = functions.pubsub
 ...
 */
exports.resetMonthlyTeamHope = functions.pubsub
...
async function isAdmin(uid) {
    ...
    return adminDoc.exists && ((_a = adminDoc.data()) === null || _a === void 0 ? void 0 : _a.is_active) === true;
}
/**
 ...
 */
exports.calculateAdminStats = functions.pubsub
    ...
    try {
        ...
        const oneDayAgo = admin.firestore.Timestamp.fromDate(new Date(now.getTime() - 24 * 60 * 60 * 1000));
...
exports.manualCalculateAdminStats = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await logAdminAction(context.auth.uid, "calculate_stats", "admin_stats", "current", { date: today });
        return {
            success: true,
...
exports.sendBroadcastNotification = functions.https.onCall(async (data, context) => {
    ...
    if (!title || !body) {
        throw new functions.https.HttpsError("invalid-argument", "Başlık ve içerik gereklidir.");
    }
    try {
        ...
        if (targetAudience === "premium") {
            ...
            usersQuery = usersQuery.where(admin.firestore.FieldPath.documentId(), "in", leaderIds.slice(0, 10) // Firestore 'in' limiti
            );
        }
        ...
        await logAdminAction(context.auth.uid, "send_broadcast", "broadcast_notifications", broadcastRef.id, { title, targetAudience, successCount, failureCount });
        return {
            success: true,
...
exports.toggleUserBan = functions.https.onCall(async (data, context) => {
    ...
    if (!userId) {
        throw new functions.https.HttpsError("invalid-argument", "Kullanıcı ID gereklidir.");
    }
    try {
        ...
        await logAdminAction(context.auth.uid, isBanned ? "ban_user" : "unban_user", "users", userId, { reason });
        return {
            success: true,
            message: isBanned
    ...
    try {
        ...
        donationsSnapshot.forEach((doc) => {
            ...
            donations.push({
                ...
                createdAt: ((_c = (_b = (_a = data.created_at) === null || _a === void 0 ? void 0 : _a.toDate) === null || _b === void 0 ? void 0 : _b.call(_a)) === null || _c === void 0 ? void 0 : _c.toISOString()) || null,
            });
 ...
 */
exports.resetMonthlyCarryoverSteps = functions.pubsub
...
exports.triggerMonthlyReset = functions.https.onCall(async (data, context) => {
    ...
    if (!adminDoc.exists || !((_b = adminDoc.data()) === null || _b === void 0 ? void 0 : _b.is_active)) {
        throw new functions.https.HttpsError("permission-denied", "Bu işlem için admin yetkisi gerekiyor");
    }
    try {
        const usersSnapshot = await db.collection("users").get();
        const batch = db.batch();
 ...
 */
exports.distributeMonthlyLeaderboardRewards = functions.pubsub
 ...
 */
async function distributeStepRewards(monthStart, monthEnd, yearMonth, rewards) {
    const validTypes = ["step_conversion", "step_conversion_2x", "carryover_conversion"];
    const userSteps = {};
    const processedDocs = new Set(); // Duplicate önleme
 ...
 */
async function distributeDonationRewards(monthStart, monthEnd, yearMonth, rewards) {
    const userDonations = {};
    const processedDocs = new Set(); // Duplicate önleme
    // Helper: Tarih aralığında mı kontrol et
    const isInDateRange = (data) => {
        let logDate = null;
        if (data.created_at) {
 ...
 * Flutter ile tutarlı: hem activity_type hem action_type, hem created_at hem timestamp
 */
async function distributeTeamRewards(monthStart, monthEnd, yearMonth, rewards) {
    var _a, _b;
    const teamDonations = {};
    const processedDocs = new Set(); // Duplicate önleme
    // Helper: Tarih aralığında mı kontrol et
    ...
    // Helper: Bağış işle
    const processDonation = async (doc) => {
        var _a;
        if (processedDocs.has(doc.id))
            return;
        const data = doc.data();
        ...
        const teamId = (_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.current_team_id;
        if (teamId) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        const teamDoc = await db.collection("teams").doc(teamId).get();
        const memberIds = ((_a = teamDoc.data()) === null || _a === void 0 ? void 0 : _a.member_ids) || [];
        const teamName = ((_b = teamDoc.data()) === null || _b === void 0 ? void 0 : _b.name) || "Takım";
 ...
 */
async function sendRewardNotification(userId, category, rank, rewardSteps) {
    const rankEmojis = ["🥇", "🥈", "🥉"];
    const categoryNames = {
 ...
 */
async function sendTeamRewardNotification(userId, teamName, rank, rewardSteps) {
    const rankEmojis = ["🥇", "🥈", "🥉"];
...
var admob_reporter_1 = require("./admob-reporter");
Object.defineProperty(exports, "fetchAdMobRevenue", { enumerable: true, get: function () { return admob_reporter_1.fetchAdMobRevenue; } });
Object.defineProperty(exports, "fetchAdMobRevenueManual", { enumerable: true, get: function () { return admob_reporter_1.fetchAdMobRevenueManual; } });
// ==================== AYLIK HOPE DEĞERİ HESAPLAMA ====================
var monthly_hope_calculator_1 = require("./monthly-hope-calculator");
Object.defineProperty(exports, "calculateMonthlyHopeValue", { enumerable: true, get: function () { return monthly_hope_calculator_1.calculateMonthlyHopeValue; } });
