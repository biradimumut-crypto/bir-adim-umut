...
class BadgeService {
  ...
  /// NOT: Sadece REEL adımlar dahil edilir (step_conversion, step_conversion_2x, carryover_conversion)
  /// Bonus adımlar (referral, leaderboard, team) rozet hesaplamasına dahil EDİLMEZ
  Future<int> _calculateLifetimeSteps(String uid) async {
    try {
      int totalSteps = 0;
      
      // Sadece REEL adım türleri - bonus adımlar dahil değil
      const realStepTypes = {
        'step_conversion',      // Günlük adım dönüşümü
        'step_conversion_2x',   // 2x bonus dönüşüm (premium)
        'carryover_conversion', // Taşınan adım dönüşümü
      };
      // NOT: bonus_conversion, leaderboard_bonus_conversion, team_bonus_conversion
      // dahil edilmiyor çünkü bunlar fiziksel adım değil
      
      // 1. Global activity_logs koleksiyonundan oku
      ...
      for (final doc in globalLogs.docs) {
        ...
        // Sadece reel adım türlerini dahil et
        if (realStepTypes.contains(activityType)) {
      ...
      for (final doc in userLogs.docs) {
        ...
        // Sadece reel adım türlerini dahil et
        if (realStepTypes.contains(actionType)) {
          totalSteps += (data['steps_converted'] ?? data['steps'] ?? 0) as int;
        }
      }
  ...
  Future<bool> awardBadge(String uid, String badgeId) async {
    try {
      ...
      // 🏅 Aktivite geçmişine rozet kazanımını kaydet
      ...
      if (badgeDef != null) {
        final activityData = {
          ...
          'user_id': uid,
          'badge_id': badgeId,
          'badge_name': _getBadgeNameTr(badgeId), // Türkçe rozet adı
          'badge_icon': badgeDef.icon,
          'created_at': FieldValue.serverTimestamp(),
        };
        
        // Global activity_logs koleksiyonuna yaz
        await _firestore.collection('activity_logs').add(activityData);
        
        // User subcollection'a da yaz (daha güvenilir)
        await _firestore
            .collection('users')
            .doc(uid)
            .collection('activity_logs')
