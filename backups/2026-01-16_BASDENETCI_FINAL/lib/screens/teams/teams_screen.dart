...
class _TeamsScreenState extends State<TeamsScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadJoinRequests() async {
    ...
    try {
      final requestsSnapshot = await _firestore
          ...
          .where('notification_type', isEqualTo: 'join_request')
          .where('notification_status', isEqualTo: 'pending')
          .get();
  ...
  Future<void> _rejectInvite(Map<String, dynamic> invite) async {
    ...
    try {
      ...
      final uid = _auth.currentUser?.uid;
      await _firestore.collection('users').doc(uid).collection('notifications').doc(invite['notificationId']).update({
  ...
  Future<void> _acceptJoinRequest(Map<String, dynamic> request, StateSetter setDialogState) async {
    ...
    try {
      ...
      // 4. Notification'ı accepted olarak güncelle
      final leaderUid = _auth.currentUser?.uid;
      batch.update(_firestore.collection('users').doc(leaderUid).collection('notifications').doc(notificationId), {
        'notification_status': 'accepted',
        'responded_at': Timestamp.now(),
      });
  ...
  Future<void> _rejectJoinRequest(Map<String, dynamic> request, StateSetter setDialogState) async {
    ...
    try {
      // Notification'ı rejected olarak güncelle
      final leaderUid = _auth.currentUser?.uid;
      await _firestore.collection('users').doc(leaderUid).collection('notifications').doc(notificationId).update({
        'notification_status': 'rejected',
        'responded_at': Timestamp.now(),
      });
  ...
  Future<void> _joinTeam(String code) async {
    ...
    try {
      ...
      await _firestore.collection('users').doc(leaderUid).collection('notifications').add({
