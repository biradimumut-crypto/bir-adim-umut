...
class _ProfileScreenState extends State<ProfileScreen> with WidgetsBindingObserver {
              ...
              builder: (context, lang, _) => Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    // Takım bilgisi - Real-time Firestore'dan al
                    StreamBuilder<DocumentSnapshot>(
                      stream: FirebaseFirestore.instance
                          .collection('users')
                          .doc(FirebaseAuth.instance.currentUser?.uid)
                      ...
                      builder: (context, snapshot) {
                        ...
                        return _buildProfileStatWithImage(
                          lang.team,
                          hasTeam ? lang.hasTeam : lang.noTeam,
                ...
                children: [
                  ...
                  Consumer<ThemeProvider>(
                    builder: (context, themeProvider, child) {
                      final themeText = lang.isTurkish 
                          ? (themeProvider.isSystemMode ? 'Sistem' : (themeProvider.isDarkMode ? 'Koyu' : 'Açık'))
                          : (themeProvider.isSystemMode ? 'System' : (themeProvider.isDarkMode ? 'Dark' : 'Light'));
                      return _buildMenuItem(
                        icon: themeProvider.isDarkMode ? Icons.dark_mode : (themeProvider.isLightMode ? Icons.light_mode : Icons.brightness_auto),
                        title: lang.isTurkish ? 'Tema: $themeText' : 'Theme: $themeText',
                        onTap: () {
                          _showThemeSelectionDialog(context, lang, themeProvider);
                        },
                      );
                    },
                  ),
  ...
  Future<void> _handleLogout() async {
    ...
    if (confirm == true) {
      // Tema tercihini local'e kaydet (logout sonrası için)
      try {
        final themeProvider = Provider.of<ThemeProvider>(context, listen: false);
        await themeProvider.onUserLogout();
      } catch (e) {
        debugPrint('Tema logout senkronizasyonu hatası: $e');
      }
  ...
  /// Tema seçim dialog'u
  void _showThemeSelectionDialog(BuildContext context, LanguageProvider lang, ThemeProvider themeProvider) {
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                lang.isTurkish ? 'Tema Seçimi' : 'Theme Selection',
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 20),
              _buildThemeOption(
                ...
                lang.isTurkish ? 'Sistem' : 'System',
                lang.isTurkish ? 'Cihaz ayarlarına göre' : 'Follow device settings',
                lang,
              ),
              _buildThemeOption(
                context,
                ...
                ThemeMode.light,
                Icons.light_mode,
                lang.isTurkish ? 'Açık Tema' : 'Light Theme',
                lang.isTurkish ? 'Aydınlık görünüm' : 'Bright appearance',
                lang,
              ),
              _buildThemeOption(
                context,
                ...
                ThemeMode.dark,
                Icons.dark_mode,
                lang.isTurkish ? 'Koyu Tema' : 'Dark Theme',
                lang.isTurkish ? 'Karanlık görünüm' : 'Dark appearance',
                lang,
              ),
              const SizedBox(height: 10),
            ],
  ...
  Widget _buildThemeOption(
    ...
    return ListTile(
      ...
      onTap: () async {
        Navigator.pop(context);
        final success = await themeProvider.setThemeMode(mode);
        if (!success && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(lang.isTurkish 
                  ? 'Tema kaydedilemedi, lütfen tekrar deneyin' 
...
class _ActivityHistoryPageState extends State<ActivityHistoryPage> {
  Map<String, String> _otherUserNames = {};
  Set<String> _fetchedUserIds = {}; // 🔄 Zaten çekilen ID'ler (flickering önlemek için)
  Map<String, String> _teamNames = {}; // 🏢 Takım adları cache
  Set<String> _fetchedTeamIds = {}; // 🔄 Zaten çekilen takım ID'ler
  ...
  /// Diğer kullanıcı isimlerini toplu çek (sadece yeni ID'ler için)
  void _fetchOtherUserNamesIfNeeded(List<Map<String, dynamic>> activities) {
    final Set<String> userIds = {};
    
    for (var activity in activities) {
      final otherId = activity['other_user_id'] as String?;
      final newMemberId = activity['new_member_id'] as String?;
      if (otherId != null && otherId.isNotEmpty) userIds.add(otherId);
      if (newMemberId != null && newMemberId.isNotEmpty) userIds.add(newMemberId);
    }
    
    // Sadece yeni ID'ler varsa fetch yap (flickering önle)
    final newIds = userIds.difference(_fetchedUserIds);
    if (newIds.isEmpty) return;
    
    // Async olarak çek ama build'i bloklamadan
    _fetchUserNames(newIds.toList());
  }
  
  /// Kullanıcı isimlerini async olarak çek
  Future<void> _fetchUserNames(List<String> userIds) async {
    ...
    for (var i = 0; i < userIds.length; i += 10) {
      ...
      try {
        ...
        for (var doc in snapshot.docs) {
          names[doc.id] = doc.data()['full_name'] ?? 'Kullanıcı';
        }
      } catch (e) {
    ...
    if (mounted && names.isNotEmpty) {
      setState(() {
        _otherUserNames.addAll(names);
        _fetchedUserIds.addAll(userIds);
      });
    }
  }

  /// Diğer kullanıcı ismini al (güncel veya fallback)
  String? _getOtherUserName(Map<String, dynamic> activity) {
    final otherId = activity['other_user_id'] as String?;
    final newMemberId = activity['new_member_id'] as String?;
    
    if (otherId != null && _otherUserNames.containsKey(otherId)) {
      return _otherUserNames[otherId];
    }
    if (newMemberId != null && _otherUserNames.containsKey(newMemberId)) {
      return _otherUserNames[newMemberId];
    }
    
    // Fallback: kayıtlı eski isim
    return activity['other_user_name'] ?? activity['new_member_name'];
  }
  
  /// Takım adlarını toplu çek (sadece yeni ID'ler için)
  void _fetchTeamNamesIfNeeded(List<Map<String, dynamic>> activities) {
    final Set<String> teamIds = {};
    
    for (var activity in activities) {
      final teamId = activity['team_id'] as String?;
      // team_name yoksa veya 'Takım' ise gerçek adı çek
      if (teamId != null && teamId.isNotEmpty) {
        final existingName = activity['team_name'] as String?;
        if (existingName == null || existingName == 'Takım' || existingName == 'Team') {
          teamIds.add(teamId);
        }
      }
    }
    
    // Sadece yeni ID'ler varsa fetch yap
    final newIds = teamIds.difference(_fetchedTeamIds);
    if (newIds.isEmpty) return;
    
    _fetchTeamNames(newIds.toList());
  }
  
  /// Takım isimlerini async olarak çek
  Future<void> _fetchTeamNames(List<String> teamIds) async {
    ...
    for (var i = 0; i < teamIds.length; i += 10) {
      ...
      try {
        ...
        for (var doc in snapshot.docs) {
          names[doc.id] = doc.data()['name'] ?? 'Takım';
        }
      } catch (e) {
        debugPrint('Team name fetch error: $e');
      }
    }
    
    if (mounted && names.isNotEmpty) {
      setState(() {
        _teamNames.addAll(names);
        _fetchedTeamIds.addAll(teamIds);
      });
    }
  }
  
  /// Takım adını al (cache veya activity'den)
  String _getTeamName(Map<String, dynamic> activity, dynamic lang) {
    final teamId = activity['team_id'] as String?;
    
    // Cache'de varsa onu kullan
    if (teamId != null && _teamNames.containsKey(teamId)) {
      return _teamNames[teamId]!;
    }
    
    // Activity'de kayıtlı varsa onu kullan
    final storedName = activity['team_name'] as String?;
    if (storedName != null && storedName != 'Takım' && storedName != 'Team') {
      return storedName;
    }
    
    // Fallback
    return lang.isTurkish ? 'Takım' : 'Team';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        // User subcollection'dan oku (daha güvenilir)
        stream: FirebaseFirestore.instance
            .collection('users')
            .doc(widget.userId)
            .collection('activity_logs')
        ...
        builder: (context, snapshot) {
          ...
          final activityDataList = activities.map((e) => e.data() as Map<String, dynamic>).toList();
          
          // Diğer kullanıcı isimlerini çek (sadece yeni ID'ler için - flickering önle)
          _fetchOtherUserNamesIfNeeded(activityDataList);
          // Takım isimlerini çek (eski kayıtlarda team_name yoksa)
          _fetchTeamNamesIfNeeded(activityDataList);
  ...
  Widget _buildActivityItem(BuildContext context, Map<String, dynamic> activity) {
    ...
    switch (type) {
      ...
      case 'team_referral_bonus':
        ...
        final newMemberName = _getOtherUserName(activity);
      ...
      case 'referral_bonus':
        ...
        final refUserName = _getOtherUserName(activity);
      ...
      case 'team_joined':
        icon = Icons.group_add;
        color = Colors.green;
        final teamName = _getTeamName(activity, lang);
        title = lang.teamJoinedActivity;
        subtitle = teamName;
        break;
      case 'team_join_bonus':
        icon = Icons.card_giftcard;
        color = const Color(0xFF27AE60); // Yeşil
        final teamJoinBonus = activity['bonus_steps'] ?? 100000;
        final teamJoinName = _getTeamName(activity, lang);
        title = lang.isTurkish ? 'Takıma Katılım Bonusu' : 'Team Join Bonus';
        subtitle = lang.isTurkish 
            ? '$teamJoinName takımına katıldınız, +${_formatActivityNumber(teamJoinBonus)} bonus adım kazandınız' 
            : 'Joined $teamJoinName team, +${_formatActivityNumber(teamJoinBonus)} bonus steps earned';
        break;
      case 'team_contribution_bonus':
        icon = Icons.volunteer_activism;
        color = const Color(0xFF6EC6B5); // Turkuaz
        final contribBonus = activity['bonus_steps'] ?? 100000;
        final contribTeamName = _getTeamName(activity, lang);
        title = lang.isTurkish ? 'Takıma Katkı' : 'Team Contribution';
        subtitle = lang.isTurkish 
            ? '$contribTeamName takımınıza +${_formatActivityNumber(contribBonus)} bonus adım kazandırdınız' 
            : 'You contributed +${_formatActivityNumber(contribBonus)} bonus steps to $contribTeamName team';
        break;
      case 'team_new_member_bonus':
        icon = Icons.person_add_alt_1;
        color = const Color(0xFF9B59B6); // Mor
        final newMemberBonus = activity['bonus_steps'] ?? 100000;
        final newMemberTeamName = _getTeamName(activity, lang);
        // Güncel ismi tercih et, yoksa kayıtlı ismi kullan
        final newMemberUserName = _getOtherUserName(activity) ?? (lang.isTurkish ? 'Yeni üye' : 'New member');
        title = lang.isTurkish ? 'Yeni Üye Bonusu' : 'New Member Bonus';
        subtitle = lang.isTurkish 
            ? '$newMemberUserName, $newMemberTeamName takımınıza +${_formatActivityNumber(newMemberBonus)} adım kazandırdı' 
            : '$newMemberUserName contributed +${_formatActivityNumber(newMemberBonus)} steps to $newMemberTeamName team';
        break;
      case 'badge_earned':
        // 🏅 Rozet kazanımı
        final badgeIcon = activity['badge_icon'] ?? '🏅';
        final badgeName = activity['badge_name'] ?? (lang.isTurkish ? 'Rozet' : 'Badge');
        icon = Icons.workspace_premium;
        color = const Color(0xFFF2C94C); // Altın
        title = lang.isTurkish ? 'Rozet Kazanıldı!' : 'Badge Earned!';
        subtitle = '$badgeIcon $badgeName';
        break;
      case 'team_created':
