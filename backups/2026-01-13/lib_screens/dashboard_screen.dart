...
class _DashboardScreenState extends State<DashboardScreen> with WidgetsBindingObserver {
  ...
  // 🔄 Auto-refresh Timer (Health API için)
  ...
  void initState() {
    ...
    _startAutoRefreshTimer(); // 🔄 Adım verilerini otomatik güncelle
    // Dashboard açıldığında yeni rozetleri kontrol et (delay ile)
    _checkNewBadges(withDelay: true);
  }
  
  /// 🔄 Her 30 saniyede bir Health API'den adımları güncelle
  void _startAutoRefreshTimer() {
    _autoRefreshTimer?.cancel();
    _autoRefreshTimer = Timer.periodic(const Duration(seconds: 30), (timer) {
      if (mounted && _selectedIndex == 0) {
        // Sadece dashboard sekmesindeyken güncelle
        _syncHealthData();
      }
    });
  }
  
  /// Health API'den adımları senkronize et (sessiz - loading gösterme)
  Future<void> _syncHealthData() async {
    ...
    try {
      ...
      if (syncResult['success'] == true && mounted) {
        // Firestore stream otomatik güncelleyecek, ama weekly data'yı da güncelle
  ...
  void dispose() {
    ...
    _autoRefreshTimer?.cancel(); // Auto-refresh timer iptal et
  ...
  Widget _buildWeeklyContent(LanguageProvider lang, int maxValue) {
    ...
    return Column(
      ...
      children: [
        ...
        // Diğer tab'larla yükseklik dengelemesi
