...
export const fetchAdMobRevenue = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      const dateKey = `${yesterday.getFullYear()}-${String(
 ...
 */
export const fetchAdMobRevenueManual = functions.https.onCall(
  async (data, context) => {
    // Admin kontrolü
    if (!context.auth) {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Rapor çekilemedi"
      );
    }
  }
);
