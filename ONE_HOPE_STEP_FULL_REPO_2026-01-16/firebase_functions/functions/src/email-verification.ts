...
export const sendVerificationCode = onCall(
  ...
  try {
    ...
    if (!email) {
      throw new HttpsError("failed-precondition", "Email adresi bulunamadı");
    }

    // Zaten doğrulanmış mı kontrol et
    const userDoc = await db.doc(`users/${uid}`).get();
    if (userDoc.exists && userDoc.data()?.email_verified === true) {
      throw new HttpsError("already-exists", "Email zaten doğrulanmış");
    }
    ...
    const recentCodes = await db.collection("verification_codes")
      ...
      .where("created_at", ">", admin.firestore.Timestamp.fromDate(
        new Date(Date.now() - 60 * 1000) // 1 dakika önce
      ))
      .get();
...
export const verifyEmailCode = onCall(
  ...
  try {
    ...
    if (codesSnapshot.empty) {
      throw new HttpsError("not-found", "Doğrulama kodu bulunamadı. Yeni kod isteyin.");
    }
    ...
    throw new HttpsError("internal", "Doğrulama başarısız");
  }
});
