 ...
 */
async function deleteCollectionChunked(
 ...
 */
async function deleteQueryResultsChunked(
...
export const deleteAccount = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // 3.1 Notifications
    deletionReport.notifications = await deleteCollectionChunked(
      db.collection("users").doc(uid).collection("notifications")
    );
    console.log(`  ✓ Notifications silindi: ${deletionReport.notifications}`);
    
    // 3.2 Badges
    deletionReport.badges = await deleteCollectionChunked(
      db.collection("users").doc(uid).collection("badges")
    );
    console.log(`  ✓ Badges silindi: ${deletionReport.badges}`);
    
    // 3.3 User activity_logs (subcollection)
    deletionReport.userActivityLogs = await deleteCollectionChunked(
