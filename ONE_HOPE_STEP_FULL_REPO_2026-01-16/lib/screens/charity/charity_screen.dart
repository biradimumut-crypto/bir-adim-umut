...
class _CharityScreenState extends State<CharityScreen> {
  ...
  Future<void> _processDonationNew(CharityModel charity, double amount) async {
    ...
    try {
      // 🔒 Atomik transaction ile tüm bağış işlemlerini gerçekleştir
      await firestore.runTransaction((transaction) async {
        // 📖 OKUMA AŞAMASI (Transaction'da önce tüm okumalar yapılmalı)
        
        // 1. Kullanıcı bakiyesini oku ve kontrol et
        ...
        final currentBalance = (userDoc.data()?['wallet_balance_hope'] ?? 0.0).toDouble();
        
        // Yetersiz bakiye kontrolü (transaction içinde)
        if (currentBalance < amount) {
          throw Exception('Yetersiz bakiye');
        }
        
        // 2. Charity dokümanını oku
        final charityDoc = await transaction.get(
          firestore.collection('charities').doc(charity.id)
        );
        
        if (!charityDoc.exists) {
          throw Exception('Vakıf bulunamadı');
        }
        
        // 3. Takım üyeliği kontrolü (varsa)
        DocumentSnapshot? teamMemberDoc;
        DocumentSnapshot? teamDoc;
        if (_currentUser?.currentTeamId != null) {
          teamMemberDoc = await transaction.get(
            firestore.collection('teams')
                .doc(_currentUser!.currentTeamId)
                .collection('team_members')
                .doc(uid)
          );
          teamDoc = await transaction.get(
            firestore.collection('teams').doc(_currentUser!.currentTeamId)
          );
        }
        
        // 🔍 İlk bağış kontrolü - Transaction dışında yapılmalı (where sorgusu)
        ...
        // ✏️ YAZMA AŞAMASI (Tüm okumalar bittikten sonra)
        
        // 1. Kullanıcı bakiyesini düş + istatistikleri güncelle
        transaction.update(
          firestore.collection('users').doc(uid),
          {
        ...
        transaction.set(logRef, {
          ...
          'charity_logo_url': charity.imageUrl,
        ...
        // 4. Charity'nin toplam bağışını güncelle
        final charityUpdateData = <String, dynamic>{
          'collected_amount': FieldValue.increment(amount),
        };
        if (isFirstDonation) {
          charityUpdateData['donor_count'] = FieldValue.increment(1);
        }
        transaction.update(
          firestore.collection('charities').doc(charity.id),
          charityUpdateData,
        ...
        if (_currentUser?.currentTeamId != null && teamMemberDoc != null && teamDoc != null) {
      ...
      // ✅ Transaction başarılı - rozet kontrolü (transaction dışında)
      ...
      if (mounted) {
        ...
        // Özel hata mesajları
        String errorMessage = lang.errorMsg(e.toString());
        if (e.toString().contains('Yetersiz bakiye')) {
          errorMessage = lang.isTurkish 
              ? 'Yetersiz bakiye. Daha fazla adım atarak umut kazanın!' 
              : 'Insufficient balance. Earn more hope by walking!';
        }
